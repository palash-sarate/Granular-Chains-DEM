...
def run_load_data(run_path, output_path):
    ...
    for b_idx in range(num_batches):
        ...
        if df_atoms is not None and not df_atoms.empty:
            ...
            cols_to_keep = [c for c in ['timestep', 'id', 'mol', 'x', 'y', 'z', 'mass'] if c in df_reset.columns]
    ...
    for col in ['id', 'mol', 'x', 'y', 'z', 'mass', 'timestep']:
    ...
    df_all.dropna(subset=['id', 'x', 'y', 'z', 'timestep'], inplace=True)
    df_all.set_index(['timestep', 'id'], inplace=True)
    df_all.sort_index(inplace=True)
    
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    df_all.to_parquet(output_path)
    print(f"Saved simplified simulation data to: {output_path}")
